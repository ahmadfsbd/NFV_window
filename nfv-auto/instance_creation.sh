#!/bin/bash
#for i in {1..4}; do openstack server create --image ceph_med_vm_snap --flavor ceph-2vol --network storage-net --security-group c11a0ebb-22bb-4658-9804-c20d0053412a --key-name ceph-key --availability-zone nova0  ceph-net1-nova0-vm-$i && echo "Volume attached to server ceph-net1-nova0-vm-$i successfully"; done
...
#for i in {1..4}; do openstack server create --image ceph_med_vm_snap --flavor ceph-2vol --network storage-net --security-group c11a0ebb-22bb-4658-9804-c20d0053412a --key-name ceph-key --availability-zone nova2  ceph-net1-nova2-vm-$i && echo "Volume attached to server ceph-net1-nova2-vm-$i successfully"; done

#for i in {1..6}; do openstack server create --image ceph_med_vm_snap --flavor ceph-2vol --network storage-net2 --security-group c11a0ebb-22bb-4658-9804-c20d0053412a --key-name ceph-key --availability-zone nova0  ceph-net2-nova0-vm-$i && echo "server created ceph-net2-nova0-vm-$i successfully"; done
...
#for i in {1..30}; do openstack volume create --size 1024 --bootable $((i+30)) && echo "Volume created $((i+30))"; done

#for i in {1..4}; do openstack server add volume ceph-net1-nova0-vm-$i $((i+30)) && echo "Volume attached to server ceph-net1-nova2-vm-$i"; done

#for i in {1..4}; do openstack server add volume ceph-net1-nova1-vm-$i $((i+34)) && echo "Volume attached to server ceph-net1-nova2-vm-$i"; done

#for i in {1..4}; do openstack server add volume ceph-net1-nova2-vm-$i $((i+38)) && echo "Volume attached to server ceph-net1-nova2-vm-$i"; done
...
for i in {0..18}; do openstack server add floating ip "${net2_vm_list[$((i))]}" "${net2_floating_ip[$((i))]}" && echo "openstack server add floating ip "${net1_floating_ip[$((i))]}" "${net1_vm_list[$((i))]}""; done
